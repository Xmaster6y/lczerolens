...
class ActivationsStore:
    ...
    def from_config(
        ...
        if override_dataset is None and cfg.dataset_path == "":
            raise ValueError("You must either pass in a dataset or specify a dataset_path in your configutation.")
    ...
    def __init__(
        ...
        if "tokens" in dataset_sample.keys():
            ...
            raise ValueError("Dataset must have a 'tokens', 'input_ids', 'text', or 'problem' column.")
    ...
    def _iterate_tokenized_sequences(self) -> Generator[torch.Tensor, None, None]:
        ...
        # If the dataset isn't tokenized, we'll tokenize, concat, and batch on the fly
            ...
            yield from concat_and_batch_sequences(
                ...
                sequence_separator_token_id=(bos_token_id if self.prepend_bos else None),
            )
    ...
    def check_cached_activations_against_config(self):
        if self.cached_activations_path is not None:  # EDIT: load from multi-layer acts
            ...
            first_buffer = self.load_buffer(f"{self.cached_activations_path}/{self.next_cache_idx}.safetensors")
    ...
    def get_batch_tokens(self, batch_size: int | None = None, raise_at_epoch_end: bool = False):
        """
    ...
    def get_activations(self, batch_tokens: torch.Tensor):
        ...
        if self.hook_head_index is not None:
            stacked_activations[:, :, 0] = layerwise_activations[self.hook_name][:, :, self.hook_head_index]
        elif layerwise_activations[self.hook_name].ndim > 3:  # if we have a head dimension
            try:
                stacked_activations[:, :, 0] = layerwise_activations[self.hook_name].view(n_batches, n_context, -1)
            except RuntimeError as e:
                print(f"Error during view operation: {e}")
                print("Attempting to use reshape instead...")
                stacked_activations[:, :, 0] = layerwise_activations[self.hook_name].reshape(n_batches, n_context, -1)
        else:
    ...
    @torch.no_grad()
    def get_buffer(self, n_batches_in_buffer: int, raise_on_epoch_end: bool = False) -> torch.Tensor:
        """
        ...
        for refill_batch_idx_start in refill_iterator:
            ...
            refill_batch_tokens = self.get_batch_tokens(raise_at_epoch_end=raise_on_epoch_end).to(
