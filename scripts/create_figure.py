...
def add_plot(
    ...
    with doc.create(Figure()) as fig:
        ...
        verbatim = NoEscape(r"\storechessboardstyle{8x8}{maxfield=h8,showmover=true}")
        doc.append(verbatim)
        ...
        with doc.create(
            ...
            if current_piece_pos is not None:
                ...
                markfields = "{{" + current_piece_pos + "},{" + next_move + "}}"
                ...
                chessboard_fen = NoEscape(
                    rf"\chessboard[style=8x8,setfen={label}," "showmover=true,pgfstyle=straightmove,color=green,]"
                )
