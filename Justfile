...
tests-unit:
	uv run --group dev --group conformance --extra hub pytest tests -m 'unit' --cov=src --cov-report=term-missing -s -v
...
tests-slow:
	uv run --group dev --group conformance --extra hub pytest tests -m 'slow or integration' --cov=src --cov-report=term-missing --cov-branch --cov-report=xml --junitxml=junit.xml -o junit_family=legacy -s -v
